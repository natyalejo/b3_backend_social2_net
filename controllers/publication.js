...
let itemsPerPage = req.query.limit ? parseInt(req.query.page, 10) : 5;
...
const publication = await Publication.paginate({user_id: userId }, options);

if(!publication.docs || publication.docs.length <= 0){
  return res.status(404).send({
    status: "error",
...
publications: publication.docs,
total: publication.totalDocs,
pages: publication.totalPages,
page: publication.page,
limit_items_ppage: publication.limit

})


} catch (error) {
  console.log(`Error al modtrar la publicación: ${ error }`);
  return res.status(500).send({
    status: "error",
    message: "Error al mostrar la publicación"
  });
